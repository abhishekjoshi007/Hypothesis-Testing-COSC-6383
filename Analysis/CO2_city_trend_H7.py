...
# Merge the global mean CO2/MEI back into all cities
...
for city, df in city_dfs.items():
    ...
    df[['temperature_c','co2_ppm']].dropna().to_csv(f"{OUT_DIR}/{city.replace(' ','_')}_aligned.csv")
...
plt.ylabel("β_CO₂")
plt.title("CO₂ Effect on Temperature (After ENSO Control)")
...
plt.title("Combined Temperature Trends Across Cities")
...
plt.xlabel("Year")
plt.ylabel("CO₂ (ppm)")
plt.title("Global Mean CO₂ Trend (Shared Across All Cities)")
plt.grid(alpha=0.3)
plt.tight_layout()
...
for c, df in city_dfs.items():
    if 'co2_ppm' in df:
        plt.plot(df['time'], df['co2_ppm'], label=c, lw=1.4)
plt.xlabel("Year")
plt.ylabel("CO₂ (ppm)")
plt.title("City-Specific CO₂ Trends (If Locally Differentiated)")
plt.legend(loc='best', fontsize=8)
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/H7_combined_CO2_citywise.png", dpi=400)
plt.close()

print(f"\nAll results and plots saved in: {OUT_DIR}")
